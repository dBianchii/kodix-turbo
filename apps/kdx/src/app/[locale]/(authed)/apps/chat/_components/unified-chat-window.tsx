...
export function UnifiedChatWindow({
  ...
  const { createSessionWithMessage, isCreating } = useAutoCreateSession({
    onSuccess: (newSessionId) => {
      console.log("🎉 [UNIFIED_CHAT] Nova sessão criada:", newSessionId);
  ...
  useEffect(() => {
    ...
    if (data?.messages) {
      ...
      if (
        ...
        console.log(
          "🤖 [UNIFIED_CHAT] Detectada nova sessão com apenas mensagem do usuário, processando resposta da IA...",
        );
  ...
  const handleNewMessage = async (message: string) => {
    ...
    console.log("🚀 [UNIFIED_CHAT] Criando nova sessão com mensagem:", message);
    setError(null);
