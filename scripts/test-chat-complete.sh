...
run_test_suite "api-structure" \
    "$FRONTEND_TEST_DIR/integration/api.test.ts" \
    "API Structure Tests" \
    "frontend"

# 9b. Testes de Componente Message
run_test_suite "message-component" \
    "$FRONTEND_TEST_DIR/components/message.test.tsx" \
    "Message Component Logic Tests" \
    "frontend"
...
echo "✅ Message Component Logic Tests"
