...
export type Channel = EmailChannel | PushNotificationsChannel;
export function notificationServiceFactory({
  ...
  async function sendNotifications({
    ...
    const user = await publicUserRepository.findUserById(userId);
    ...
    if (toDeleteExpoPushTokens.length)
      await publicNotificationsRepository.deleteManyExpoTokens({
        tokens: toDeleteExpoPushTokens,
      });
    if (sentMessages.length)
      await publicNotificationsRepository.createManyNotifications(sentMessages);
  }
