export default {
  home: {
    title: "Bem-vindo ao Kodix",
    description:
      "Soluções para o seu negócio. Kodix é uma plataforma multiuso que oferece uma ampla gama de soluções de negócios",
  },
  header: {
    Account: "Conta",
    Settings: "Configurações",
    "Change team": "Mudar de equipe",
    "Log out": "Sair",
    "Log in": "Entrar",
    "Sign up": "Cadastrar",
    "Invitation accepted": "Convite aceito",
    "Invitation declined": "Convite recusado",
    accept: "Aceitar",
    "invited you to": "convidei você para",
    "Add more apps": "Adicione mais apps",
    "Search app": "Pesquisar app",
  },
  signin: {
    "Or continue with": "Ou continue com",
    "Your email": "Seu email",
    "Sign in": "Entrar",
    "Sign in with Google": "Entrar com Google",
    "Sign in to your account": "Faça login em sua conta",
  },
  apps: {
    kodixCare: {
      currentShift: "Turno atual",
      onboarding: {
        goToKodixCare: "Ir para Kodix Care",
        cardDescription:
          "Para iniciar sua integração, escolha o nome do paciente",
        "You can change this later in the settings":
          "Você pode alterar isso mais tarde nas configurações",
        cardTitle: "Seja bem vindo ao Kodix Care",
      },
      "Any information": "Qualquer informação",
      "Are you sure": "Tem certeza?",
      "Edit task": "Editar tarefa",
      "Mark task as done": "Marcar tarefa como concluída",
      "Mark task as not done": "Marcar tarefa como não concluída",
      "Please inform the date and time of completion":
        "Informe a data e hora de conclusão",
      "Task marked as done": "Tarefa marcada como concluída",
      "Task marked as not done": "Tarefa marcada como não concluída",
      "Task updated": "Tarefa atualizada",
      Checkout: "Check-out",
      "Checkout shift": "Realizar check-out",
      "End previous shift and start new":
        "Terminar o turno anterior e começar um novo",
      "Previous shift exists": "O turno anterior existe",
      "Start new shift": "Iniciar novo turno",
      "Start shift": "Iniciar turno",
      "You are about to start a new shift are you sure":
        "Você está prestes a iniciar um novo turno. Tem certeza?",
      "are you sure you want to end it and start a new one":
        "Tem certeza de que deseja encerrar o turno e começar um novo?",
      appDescription: "Kodix Care é um app de cuidado",
      "No events for this day": "Não há eventos neste dia",
      "Unlock all tasks up until here":
        "Desbloquear todas as tarefas até aqui?",
      "This will unlock all tasks of this turn up until this task":
        "Isso vai desbloquear todas as tarefas deste turno até esta tarefa",
      "Successfully unlocked tasks": "Tarefas desbloqueadas com sucesso!",
    },
    todo: {
      appDescription: "Todo é um app de anotações",
    },
    calendar: {
      appDescription: "Calendário é um app de calendário",
      "Create event": "Criar Evento",
      "New event": "Novo evento",
      "Add description": "Adicionar descrição",
      "Event title": "Título do evento",
      "All events": "Todos os eventos",
      "Custom recurrence": "Recorrência personalizada",
      "Delete event": "Excluir evento",
      "Edit event": "Editar evento",
      "Exclude current event": "Excluir evento atual",
      "Pick a date": "Escolha uma data",
      "Select a recurrence": "Selecione uma recorrência",
      "This and future events": "Este evento e futuros",
      "This event": "Este evento",
    },
    cupom: {
      Cupom: "Cupom",
    },
  },
  Updating: "Atualizando",
  Date: "Data",
  Details: "Detalhes",
  "Done at": "Feito em",
  "No results": "Nenhum resultado",
  Save: "Salvar",
  Saving: "Salvando",
  Title: "Título",
  "You are about to finish your shift and checkout Are you sure":
    "Você está prestes a terminar seu turno e realizar check-out. Tem certeza?",
  Calendar: "Calendário",
  "Kodix Care": "Kodix Care",
  Todo: "Todo",
  At: "No",
  Cancel: "Cancelar",
  Custom: "Personalizado",
  Description: "Descrição",
  "Doesnt repeat": "Não se repete",
  Ends: "Termina",
  From: "De",
  Never: "Nunca",
  None: "Nenhum",
  Ok: "OK",
  "Repeat every": "Repita cada",
  Search: "Procurar",
  "Search by title": "Pesquisar por título",
  Today: "Hoje",
  Week: "Semana",
  Weeks: "Semanas",
  Year: "Ano",
  Years: "Anos",
  "Uninstall from team": "Desinstalar da equipe",
  Account: "Conta",
  App: "App",
  "Change team": "Mudar equipe",
  "Coming soon": "Em breve",
  "Error uninstalling": "Erro ao desinstalar",
  Install: "Instalar",
  Installed: "Instalado",
  "Log out": "Sair",
  Open: "Abrir",
  "Or continue with": "Ou continue com",
  Settings: "Configurações",
  "Sign in": "Entrar",
  Uninstall: "Desinstalar",
  "Your email": "Seu email",
  "are-you-sure-you-would-like-to-uninstall":
    "Tem certeza de que deseja desinstalar",
  confirm: "confirme",
  installed: "instalado",
  "open-dialog": "Abrir caixa de diálogo",
  uninstalled: "desinstalado",
  Decline: "Declínio",
  "Invitation declined": "Convite recusado",
  "New notifications": "Novas notificações",
  accept: "Aceitar",
  "invited you to": "convidou você para",
  notifications: "Notificações",
  questionmark: "?",
  Create: "Criar",
  "Create new team": "Criar nova equipe",
  Current: "Atual",
  End: "Fim",
  General: "Geral",
  "Go to first page": "Ir para a primeira página",
  "Go to last page": "Ir para a última página",
  "Go to next page": "Ir para a próxima página",
  "Go to previous page": "Ir para a página anterior",
  "In 3 days": "Em 3 dias",
  "In a week": "Em uma semana",
  "In one month": "Num mês",
  "In one year": "Em um ano",
  Leave: "Sair",
  Loading: "Carregando",
  Manage: "Gerenciar",
  Name: "Nome",
  "No apps found": "Nenhum app encontrado",
  "No rights reserved": "Nenhum direito reservado",
  "Open menu": "Abrir menu",
  Owner: "Dono",
  Page: "Página",
  "Pick a date": "Pick a date",
  "Rows per page": "Linhas por página",
  Select: "Selecione",
  "Sign in to your account": "Faça login em sua conta",
  "Something went wrong please try again later":
    "Algo deu errado, tente novamente mais tarde",
  "Successfully created a new team": "Equipe criada com sucesso",
  Team: "Equipe",
  Teams: "Equipes",
  Tomorrow: "Amanhã",
  account: {
    "Account settings": "Configurações de Conta",
    "Display Name": "Nome de exibição",
    "Manage the teams you are a part of or create a new one":
      "Gerencie as equipes que você",
    "Please use 32 characters at maximum":
      "Por favor use no máximo 32 caracteres.",
    "User removed from team": "Usuário removido da equipe",
    "please-enter-your-full-name-or-a-display-name-you-are-comfortable-with":
      "Digite seu nome completo ou um nome de exibição com o qual você se sinta confortável.",
  },
  "create-a-new-team-and-invite-your-team-members":
    "Crie uma nova equipe e convide os membros da sua equipe",
  created: "criada",
  de: "de",
  "rows selected": "linha(s) selecionada(s).",
  "team-name": "Nome da equipe",
  settings: {
    "These are your installed apps": "Estes são seus apps instalados",
  },
  Active: "Ativo",
  Apps: "Apps",
  Members: "Membros",
  "Not found": "Não encontrado",
  "Please use 32 characters at maximum": "Use no máximo 32 caracteres",
  Roles: "Papéis",
  "Select your app": "Selecione seu app",
  "Team Settings": "Configurações da equipe",
  "Team name": "Nome da equipe",
  "Team name saved successfully": "Nome da equipe salvo com sucesso",
  "This is your teams visible name": "Este é o nome visível da sua equipe",
  User: "Do utilizador",
  "Your installed apps": "Seus apps instalados",
  "No results found": "Nenhum resultado encontrado",
  Permission: "Permissão",
  "This app does not have any permissions":
    "Este app não tem nenhuma permissão",
  add: "Adicionar",
  vazio: "Vazio",
  search: "Procurar",
  enter: "Join",
  table: "Mesa",
  textarea: "Área de texto",
  system: "Sistema",
  dark: "Escuro",
  light: "Luz",
  "toggle-theme": "Alterar tema",
  seconds: "Segundos",
  minutes: "Minutos",
  hours: "Horas",
  Hello: "Olá",
  "Join teamName on Kodix": "Junte-se ao {teamName} no Kodix",
  Dark: "Escuro",
  Light: "Luz",
  System: "Sistema",
  "Toggle theme": "Alterar tema",
  emails: {
    "Has invited you to the teamName team on Kodix":
      "Convidou você para a equipe {teamName} em {site}",
    "If you were not expecting this invitation you can ignore this email":
      "Se você não esperava este convite, pode ignorar este e-mail. \nSe você está preocupado com a segurança da sua conta, responda a este e-mail para entrar em contato conosco.",
    "Join invitedByUsername on Kodix":
      "Junte-se a {invitedByUsername} no Kodix",
    "Join teamName on Kodix": "Junte-se a {teamName} em {site}",
    "Join the team": "Junte-se à equipe",
    "This invitation was intended for": "Este convite foi destinado a",
    "This invite was sent from": "Este convite foi enviado de",
    "located in": "localizado em",
  },
  "or copy and paste this URL into your browser:":
    "ou copie e cole este URL em seu navegador:",
  "Day#one": "Dia",
  "Day#other": "Dias",
  Days: "Dias",
  Every: "Todo",
  "Month#one": "Mês",
  "Month#other": "Meses",
  Months: "Meses",
  "Week#one": "Semana",
  "Week#other": "Semanas",
  "Year#one": "Ano",
  "Year#other": "Anos",
  recurrence: {
    At: "No",
  },
  tab: "Aba",
  "Change frequency": "Mudar a frequência",
  Day: "Dia",
  Month: "Mês",
  Assignee: "Cessionário",
  "Create task": "Criar tarefa",
  High: "Alto",
  Low: "Baixo",
  Medium: "Médio",
  "Add more": "Adicione mais",
  "App Store": "App Store",
  Confirm: "confirme",
  Delete: "Excluir",
  "Edit Event": "Editar evento",
  "Edit name permissions": "Editar permissões de {name}",
  "Edit name roles": "Editar papéis de {name}",
  "Failed to send invitation to people":
    "Falha ao enviar convite para {people}",
  "Invitations sent": "Convites enviados!",
  "Invitations sent to people": "Convites enviados para {people}",
  Invite: "Convidar",
  "Invite deleted": "Convite excluído",
  "Invite new members by email address":
    "Convide novos membros por endereço de e-mail",
  "Invite to team": "Convidar para a equipe",
  Invites: "Convites",
  "No invitations right now": "Nenhum convite no momento",
  "No members found": "Nenhum membro encontrado",
  Remove: "Remover",
  "Select all": "Selecionar tudo",
  "Select row": "Selecione a linha",
  Sending: "Enviando",
  "User removed from team": "Usuário removido da equipe",
  "You are about to invite the following Team members are you sure you want to continue":
    "Você está prestes a convidar os seguintes membros da equipe. Tem certeza de que deseja continuar?",
  "Email address": "Email address",
  "Delete event": "Excluir evento",
  "Edit event": "Editar evento",
  English: "Inglês",
  "Every day": "Diariamente",
  "Every month": "Todo mês",
  "Every week": "Toda semana",
  "Every year": "Todo ano",
  "No languages found": "Nenhum idioma encontrado",
  "Search language": "Pesquisar idioma",
  "Portuguese Brazil": "Português (Brasil)",
  Ended: "Terminou",
  "No shift started yet": "Nenhum turno começou ainda",
  Start: "Começar",
  "Best regards Kodix team custom": "Atenciosamente, {custom}",
  "Click here to sign in": "Clique aqui para entrar",
  "If you didnt request this please ignore this email":
    "Se você não solicitou isso, ignore este e-mail.",
  "Login with magic link": "Faça login com link mágico",
  Yes: "Sim",
  Unlocking: "Desbloqueio",
} as const;
